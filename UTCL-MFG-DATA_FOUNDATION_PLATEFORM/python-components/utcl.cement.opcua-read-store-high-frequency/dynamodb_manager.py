...
async def get_all_data(plant_code):
    ...
    response = table.scan(
        ProjectionExpression="#endpoint, #TagUID, #uom, #IsDisplay, #section, #path, #IsHighFrequencyTag, #ConvertToBinary",
        ExpressionAttributeNames={
            ...
            "#section": "Section",
            "#path": "Path",
            "#IsHighFrequencyTag": "IsHighFrequencyTag",
            "#ConvertToBinary": "ConvertToBinary"
        },
        FilterExpression="#IsHighFrequencyTag = :true",
    ...
    _logger.info(f"Total Records for 5 second interval ingestion: {len(data)}")
    ...
    for item in data:
        ...
        tag = {
            ...
            "Is Digital Tag": item["Unit Of Measurement"].lower() == "state",  # Convert to lowercase and check if UOM is "state"
            # "Is Realtime": item.get("IsDisplay", False)  # Check if "IsDisplay" key is present, default to False
            "Is Realtime": True         # all tags are passed in real time as this component is used for 5 second interval ingestion
        }
