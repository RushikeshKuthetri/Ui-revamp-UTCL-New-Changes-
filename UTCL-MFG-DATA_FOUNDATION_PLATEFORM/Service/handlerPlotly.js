...
module.exports.DownloadLambdaTwo = async (event) => {
  ...
  console.log('------I M HARE IN DownloadLambdaTwo--------------')
  ...
  try {
    ...
    if (event.IsTrendChart) {
      ...
      assetData = await fetchDataBatch_mul(
        ...
        // ),
        // new Date(
        //   new Date(event.updatedEndDate).getTime() - 5.5 * 60 * 60 * 1000             // Commented to resolve the issue of Plotly  data fetch from sitewisw
        // ),
        new Date(event.updatedStartDate),    // UnCommented to resolve the issue of Plotly data fetch from sitewise by TRIDAI (11-02-26)
        new Date(event.updatedEndDate), // UnCommented to resolve the issue of Plotly data fetch from sitewise by TRIDAI (11-02-26)
        // updatedStartDate,
        // updatedEndDate,
        event.interval,
        event.isMobile,
    ...
    const plotlyData = convertToPlotly(assetData);
    if (event.IsTrendChart) {
      if (event.IsManual) {
        // console.log("Download processing In Manual");
        console.log("In if condition and data is=============================>", assetData)
        return JSON.stringify(plotlyData);
        // return JSON.stringify(assetData);
      } else {
        // console.log("Download processing In else ");
        console.log("In else condition and data is=============================>", assetData)
        const response = {
          statusCode: 200,
...
// const convertToPlotly = (assetData) => {
//   const traceMap = {};
//   const statsMap = {};

//   const colors = ["#ff7f0e", "#1f77b4", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf"]

//   // 1. Group by tagUID
//   assetData.allResults.forEach((row) => {
//     const { tagUID, tagName, timestamp, averageValue } = row;
...
//     traceMap[tagUID].x.push(timestamp);
//     traceMap[tagUID].y.push(averageValue);
//     statsMap[tagUID].values.push(averageValue);
//   });

//   // 2. Build tagGraph
//   const tagGraph = Object.values(traceMap).map((trace, index) => {
//     const combined = trace.x.map((x, i) => ({
//       x,
//       y: trace.y[i]
//     }));
...
//   // 3. Build tagTable (Min / Max / SD)
//   const tagTable = Object.values(statsMap).map((stat) => {
...
const convertToPlotly = (assetData) => {
  ...
  const colors = ["#ff7f0e", "#1f77b4", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf"];
  assetData.allResults.forEach((row) => {
    const { tagUID, tagName, timestamp, averageValue, unit } = row;
  ...
  const tagTable = Object.values(statsMap).map((stat) => {
    ...
    const variance =
      values.reduce((a, b) => a + Math.pow(b - average, 2), 0) /
      values.length;
