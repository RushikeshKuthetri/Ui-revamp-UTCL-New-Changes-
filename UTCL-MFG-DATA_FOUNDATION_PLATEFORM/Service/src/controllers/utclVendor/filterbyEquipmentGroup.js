...
module.exports = {
  filterbyEquipmentGroup: async (req, res, next) => {
    ...
    try {
      if ((eq_grp == null && pln_code == null && pln != null) || (eq_grp == null && pln == null && pln_code != null)) {
        //   console.log("block 1 only with plant", pln, eq_grp);
        ...
        if (selectedData[0].Count > 50000) {
          return res.status(400).json({
            code: 400,
            status: "failed",
            message: "Max no of data (50000) exceeded." + selectedData[0].Count + " data is fetched. Try adding equipment group or Reduce the time interval",
          });
        } else {
          const result = await client.execute(
            "utcprmfgadxiotpd-db",
            `API1_PlantData_EqGroup('${sd ? sd_ist : ""}','${ed ? ed_ist : ""}','${pln ? pln : pln_code}','','${csd ? csd_ist : ""}','${ced ? ced_ist : ""}')`
          );
          ...
          const dateISOSelectedData = selectedData.map(({ Timestamp, Created_Timestamp, ...other }) => {
        ...
        // res.status(200).send(selectedData.length);
        ...
        if (selectedData[0].Count > 50000) {
          ...
          const result = await client.execute(
            "utcprmfgadxiotpd-db",
            `API1_PlantData_EqGroup('${sd ? sd_ist : ""}','${ed ? ed_ist : ""}','${pln ? pln : pln_code}','${eq_grp}','${csd ? csd_ist : ""}','${ced ? ced_ist : ""}')`
          );
