...
const { getQuery, getQueryWithoutPagination } = require("../../helpers/dynamoDB");
const {
  getAssetPropertyHistoryValueList,
} = require("../../helpers/iotSitewise");

module.exports = {
  filterbyUIDs2: async (req, res, next) => {
    ...
    if (
      ...
      // });
        const error = new Error(`Invalid Start Date. Correct Format(YYYY-MM-DDTHH:mm:ss.SSS) with timezone offset.`);
        error.status = 400;
        return next(error);
    }
    ...
    if (
      ...
      // });
        const error = new Error(`Invalid End Date. Correct Format(YYYY-MM-DDTHH:mm:ss.SSS) with timezone offset.`);
        error.status = 400;
        return next(error);
    }
    ...
    if (pln || pln_code) {
      ...
      if (!uidArray?.length) {
        ...
        //   code: 400,
        //   status: "failed",
        //   message: "Please provide atleast one uid",
        // });
      ...
      // });
        const error = new Error(`Please enter required details. Either plant name or plant name with array of uid.`);
        error.status = 400;
        return next(error);
    }
    ...
    try {
      ...
      if (!plantName && pln_code) {
        ...
        if (plantDetails?.Items?.length) {
          let plantData = plantDetails.Items.map((itemNew) => {
            ...
            const Generic_Plant_Code = Object.values(itemNew.Generic_Plant_Code)[0];
            return {
              Plant_SAP_Code,
              Generic_Plant_Code,
            };
          ...
          // });
        const error = new Error(`No matching plant found for the provided plant sap code.`);
        error.status = 404;
        return next(error);
        }
      }
      ...
      if (!tagItems.length) {
        ...
        // });
        const error = new Error(`No matching tags found for the provided UIDs.`);
        error.status = 404;
        return next(error);
      }
      ...
      if (siteWiseData?.length) {
        ...
        if (finalResult.length > 2000000) {
          // return res.status(400).json({
          //   status: "fail",
          //   message: "Result set exceeds the 2000000 record limit",
          // });
        const error = new Error(`Result set exceeds the 2000000 record limit.`);
        error.status = 400;
        return next(error);
        }
...
///////////////////////////// OLD ADX LOGIC /////////////////////////////
        ...
        //     code: 400,
        //     status: "failed",
        //     message: "Max no of data (2000000) exceeded." + selectedData[0].Count + " data is fetched. Try adding Array of Uid or Reduce the time interval",
        //   });
        // } else {
        //   const result = await client.execute(
        //     "utcprmfgadxiotpd-db",
        //     `set notruncation; fn_API_MltUid('${newString}','${pln ? pln : pln_code}','${sd ? moment(sd).utcOffset(330).format("YYYY-MM-DDTHH:mm:ss.SSS") : ""}','${ed ? moment(ed).utcOffset(330).format("YYYY-MM-DDTHH:mm:ss.SSS") : ""}','${csd ? moment(csd).utcOffset(330).format("YYYY-MM-DDTHH:mm:ss.SSS") : ""}','${ced ? moment(ced).utcOffset(330).format("YYYY-MM-DDTHH:mm:ss.SSS") : ""}')| take 2000000`
        //   );
