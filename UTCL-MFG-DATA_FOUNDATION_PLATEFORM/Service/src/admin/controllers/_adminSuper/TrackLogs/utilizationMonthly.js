...
module.exports = {
  utilizationMonthly: async (req, res, next) => {
    ...
    try {
      ...
      if (stringModules == "") {
        Query = `exec [dbo].[SP_accesslogs_utilisation_allmodules_Monthly_fortesting_Logs] ${parameters}`;
      } else {
        Query = `exec [dbo].[SP_accesslogs_utilisation_Monthly_fortesting_Logs] ${parameters}`;
      }
      ...
      result.forEach((entry) => {
        const { Month, Year, Plant, Utilisation, Total, Module } = entry;
        if (!formattedData[Plant]) {
          formattedData[Plant] = [];
        }
        formattedData[Plant].push({ Month, Year, Total, Module, Utilisation });
      });
