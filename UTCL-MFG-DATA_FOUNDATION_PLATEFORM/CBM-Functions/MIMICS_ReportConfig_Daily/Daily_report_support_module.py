...
def fetch_daily_start_end_dt(current_time, start_time, end_time):
    daily_date = current_time.date()
    start_dt = datetime.combine(daily_date, start_time.time()).replace(tzinfo=timezone.utc)
    end_dt = datetime.combine(daily_date, end_time.time()).replace(tzinfo=timezone.utc)
...
def run_daily_time_trigger(args):
    try:
        ...
        start_dt,end_dt = fetch_daily_start_end_dt(current_time,start_time,end_time) # this is for report
        ...
        if current_time_hour_str==mail_time_hour_str:
            ...
            send_mail(recipient_list,f'Daily Report for {template_name}',f'This is a automated Daily Report Scheduled by {report_owner}',content.decode('utf-8'))
            return 1
        else:
            return 0
    except Exception as e:
