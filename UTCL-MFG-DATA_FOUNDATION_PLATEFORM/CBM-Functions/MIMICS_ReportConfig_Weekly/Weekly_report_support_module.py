...
def run_weekly_trigger(args):
    try:
        print('inside run weekly trigger function')
        ...
        start_time= single_template['StartDate'].iloc[0] # report start date
        ...
        if end_date_utc>start_time:
            ...
            # if the date difference is divisible by week worth of seconds we can start remaining portion
            if date_difference.total_seconds()%(7*24*3600)==0:
                # Now we have to take the start date with respect to our end date
                new_start_time = end_date_utc-timedelta(days=7)
                start_time_str = datetime.strftime(new_start_time,'%Y-%m-%d %H:%M:%S')# this is for report
                end_time_str = datetime.strftime(end_date_utc+timedelta(hours=23,minutes=59),'%Y-%m-%d %H:%M:%S')# Here we are adding extra time to cover that 23 hours of that day
                ...
                if mail_time_str==current_hour_str:
                    ...
                    send_mail(recipients_list,f'Weekly Report for {template_name}',f'This is a automated Report Scheduled by {report_owner}',base64_encode.decode('utf-8'))
                    return 1
                else:
                    return 0
    except Exception as e:
        print(f'Exception occured while running minutes trigger for template {template_name}:{e}')
