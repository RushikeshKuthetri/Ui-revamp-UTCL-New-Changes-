...
import pandas as pd
from Weekly_report_support_module import *
import pyodbc
import os
...
def MIMICS_ReportConfig_Weekly_main(event,context) -> None:
    utc_timestamp = datetime.utcnow().replace(
        tzinfo=timezone.utc).isoformat()
    current_time = pd.Timestamp(datetime.now()+timedelta(minutes=330),tz='UTC')
    # query = 'TagId_Wise_Report| where IsScheduler=="True" and EmailFrequency=="Weekly"|order by Timestamp desc' # fetching first type
    # report_parameters_df = connect_ADX_with_AAD_application_key_auth(query=query)
    
    query = "select * from [dbo].[TagId_Wise_Report] where IsScheduler=1 and EmailFrequency='Weekly' order by Timestamp desc"
    report_parameters_df = fetch_template_info(query)
    print(f'report_parameters_df ==> {report_parameters_df}')

    with concurrent.futures.ThreadPoolExecutor() as executor:
        for i in report_parameters_df['ReportTemplateName'].unique():
            th=executor.submit(run_weekly_trigger,[report_parameters_df,i,current_time])
            result=th.result()
            if result==1:
                print(f'Weekly Report Scheduler executed for template {i} and mail triggered')
    # if mytimer.past_due:
