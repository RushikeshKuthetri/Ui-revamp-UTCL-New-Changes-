...
def fetch_sitewise_data(tag_master_df, start_time, end_time, interval):
    """
    Fetch live IoT data from SiteWise, binned to a time interval
    """
    records = []
...
def Fn_CBM_UidWiseAvg_DataPagination_Yesterday_Scheduler(v_tagUid, v_stDate, v_enDate, v_interval, v_plant, tag_table):
    """
    Full function that emulates the Kusto logic using AWS services
    """
    ...
    tag_master_df = fetch_tag_master(tag_uid_list, plant_list, tag_table)
    ...
    print(f'sitewise_df ==> {sitewise_df}')
    if len(sitewise_df) == 0:
        return pd.DataFrame()

    tag_master_df = pd.DataFrame(tag_master_df)
    merged = pd.merge(sitewise_df, tag_master_df, left_on='Id', right_on='Tag UID')

    print(f'merged_df ==> {merged}')

    grouped = merged.groupby(['Id', 'Timestamp', 'Standard Variable Description'], as_index=False)['Value'].mean()
    print(f'merge group by id, timestamp, description ==> {grouped}')
    grouped['Value'] = grouped['Value'].round(3)

    print(f'grouped value round ==> {grouped}')

    grouped['DisplayId'] = grouped['Id'] + ' Desc- ' + grouped['Standard Variable Description']

    print(f'grouped by id and description ==> {grouped}')

    pivot_df = grouped.pivot(index='Timestamp', columns='DisplayId', values='Value').reset_index()
