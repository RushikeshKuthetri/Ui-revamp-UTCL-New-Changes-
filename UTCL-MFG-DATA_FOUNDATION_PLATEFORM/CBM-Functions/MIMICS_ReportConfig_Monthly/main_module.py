...
import pandas as pd
from Monthly_report_support_module import *
import pyodbc
import os
...
def MIMICS_ReportConfig_Monthly_main(event,context) -> None:
    print('Inside MIMICS_ReportConfig_Monthly_main function')
    utc_timestamp = datetime.utcnow().replace(
        tzinfo=timezone.utc).isoformat()
    current_time = pd.Timestamp(datetime.now()+timedelta(minutes=330),tz='UTC')
    
    query = "select * from [dbo].[TagId_Wise_Report] where IsScheduler=1 and EmailFrequency='Monthly' order by Timestamp desc"
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for i in report_parameters_df['ReportTemplateName'].unique():
            th=executor.submit(run_monthly_trigger,[report_parameters_df,i,current_time])
            result=th.result()
            if result==1:
                print(f'Monthly Report Scheduler executed for template {i} and mail triggered')
