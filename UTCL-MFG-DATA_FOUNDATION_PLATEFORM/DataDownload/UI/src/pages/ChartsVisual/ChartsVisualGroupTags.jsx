...
export const ChartsVisualGroupTags = ({ showingInTrend, data }) => {
  ...
  useEffect(() => {
    ...
    setupDashboard();
    const { token, email } = JSON.parse(localStorage.getItem("login"));
    console.log("set current dashboard: ",token, email);
  ...
  const saveDashboard = async () => {
    ...
    if (!currentDashboard || !currentDashboard.name) {
      ...
      /**
       * @params {boolean} isGroupTags - if true then save dasboard under group tags dashboard category
       * @default {boolean} isGroupTags - false
       */
      await gridRef.current.saveDashboard(true);
    }
  ...
  useEffect(() => {
    const fetchAllDashboard = async () => {
      try {
        ...
        console.debug("all dashboard Response: ", response.data);
        ...
        // filter out group tags dashboard
  ...
  const formatGroupTags = (groupTags) => {
    return groupTags.map((tag) => {
      return { label: tag.KPI_TagName, value: tag.KPI_TagName, type: tag.Type };
    });
  };
  ...
  const fetchGroupTags = async (plantSelected) => {
    try {
      ...
      const url = BASE_URL + "kpiTagDetails";
      const body = {
        plant: plant[0],
        userId: email,
      };
      ...
      const groupTags = res.data.result?.["KPI_TagNames"] || [];
  ...
  function searchGroupTags(e) {
    ...
    let filteredGroupTags = groupTagOptions.filter((tag) => {
      if (
    ...
    if (filteredGroupTags.length === 0) {
      setRecordsFoundGroupTags(false);
    } else {
      setRecordsFoundGroupTags(true);
      setSelectedGroupTagList(filteredGroupTags);
    }
  }
          ...
          >
            ...
            <div className="chart-type-wrapper">
              ...
              <div className="chart-types-selector">
                {Object.entries(chartTypesMap)
                  .filter((c) => groupTagsCharts.includes(c[0]))
                  .map(([chartType, chartTypeMap], index) => {
                    return (
                    ...
                    >
                      {tagOptions.length !== 0 && (
                        <input
                          ...
                          onChange={searchGroupTags}
                        />
                      )}
                      <ul className="list-group list-group-flush ">
                        {!recordsFoundGroupTags ||
                        plantSelected.length === 0 ? (
                          <li className="list-group-item">No Records Found</li>
                        ) : selectedGroupTagList.length !== 0 ? (
                          selectedGroupTagList.map((i) => {
                            console.log(i);
                            return (
                              <div
                                draggable="true"
                                ...
                                onTouchStart={handleDragStart}
                                onTouchEnd={handleDragOver}
                                id={JSON.stringify({ ...i, isGroup: true })}
                              >
                                <p
                                  draggable="true"
                                  onDragStart={handleDragStart}
