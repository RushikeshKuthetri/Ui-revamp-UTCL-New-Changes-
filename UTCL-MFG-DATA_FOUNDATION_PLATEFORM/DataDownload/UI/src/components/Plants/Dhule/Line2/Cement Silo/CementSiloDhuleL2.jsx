import { TextContainerWithWrapWord, BlackContainer, MimicCaret, TransparentBoxWithGate, MotorCircleDigitalTag, PipeWithCircles } from "../../../../index";
import {
  useBVTagsColorStatusForNewPlantScreens,
  useColorStatus,
  useDampersValveTagsColorStatusForNewPlantScreens,
  useFanImgsTagsColorStatusForNewPlantScreens,
  useMotorTagsColorStatusForNewPlantScreens,
} from "../../../../../utils";

export const CementSiloDhuleL2 = () => {
  return (
    <div className="CementSiloDhuleL2 w-100 h-100 position-relative">
      <div className="position-absolute single-text-1">
        <div className="d-flex justify-content-center pipeVr position-absolute pipe-1">
          <div className="trapezoid"></div>
          <div className="position-absolute single-text-2">
            <div className="dashedHr-Pipe mt-1 position-relative single-text-3"></div>

            <div className="dashedHr-Pipe mt-1 position-relative single-text-4"></div>

            <div className="dashedHr-Pipe mt-1 position-relative single-text-5"></div>
          </div>
          <div className="position-absolute single-text-6">
            <div className="dashedHr-Pipe mt-1 ml-auto position-relative single-text-3"></div>

            <div className="dashedHr-Pipe mt-1 ml-auto position-relative single-text-4"></div>

            <div className="dashedHr-Pipe mt-1 ml-auto position-relative single-text-5"></div>
          </div>
        </div>
      </div>
      <div className="position-absolute single-text-2 d-flex">
        <div className="d-flex">
          <div>
            <TextContainerWithWrapWord
              data={{
                useClass: "text-dark",
                label: "612BF1",
              }}
            />
            <div className="squareplustri d-flex align-items-end justify-content-center mt-1"></div>
            <div className="d-flex text-1">
              <MotorCircleDigitalTag className={`circle-img digital-tag ${useFanImgsTagsColorStatusForNewPlantScreens("NA")}`} label="NA" />
              <TextContainerWithWrapWord
                data={{
                  useClass: "text-dark ml-2",
                  label: "612RA1",
                }}
              />
            </div>
            <div className="yellowline-Vr  downArrow line-1"></div>
            <div className="yellowline-Hr line-2"></div>
            <div className="yellowline-Vr upArrow line-3"></div>
          </div>
        </div>
      </div>
      <div className="position-absolute  single-text-3">
        <div className="containerImage text-center ml-2">
          <div className="d-flex pt-3">
            <div>
              <MotorCircleDigitalTag className={`dot digital-tag mt-1 ml-2 ${useColorStatus("PP2_612BI1_LS")}`} label="PP2_612BI1_LS" text="L" />
              <MotorCircleDigitalTag className={`dot digital-tag mt-2 ml-2 ${useColorStatus("NA")}`} label="NA" text="L" />
            </div>
            <TextContainerWithWrapWord
              data={{
                useClass: "text-dark mt-3 ml-4",
                label: "612Bl1",
              }}
            />
          </div>
        </div>
      </div>
      <div className="position-absolute single-text-4 d-flex flex-column">
        <div className="d-flex">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mt-2 ml-2",
              label: "CG4",
            }}
          />
          <TransparentBoxWithGate
            isClickable={true}
            parentClass="mx-auto"
            squareProps={{
              tagLabel: "NA",
              squareClass: `${useColorStatus("NA")}`,
            }}
          />
        </div>
        <div className="d-flex ml-2">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mt-2 ml-2",
              label: "CG5",
            }}
          />
          <TransparentBoxWithGate
            isClickable={true}
            parentClass="mx-auto"
            squareProps={{
              tagLabel: "NA",
              squareClass: `${useColorStatus("NA")}`,
            }}
          />
        </div>
        {/* <div className="d-flex ml-3">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mt-2 ml-2",
              label: "CG6",
            }}
          />
          <TransparentBoxWithGate
            isClickable={false}
            parentClass="mx-auto"
            squareProps={{
              tagLabel: "",
              squareClass: `grey-color`,
            }}
          />
        </div> */}
      </div>
      <div className="position-absolute single-text-5 d-flex">
        <BlackContainer
          data={{
            label: "CM2_612SI1_LT",
            useClass: "greenTxt",
            unit: "",
            unitColor: "unitColor",
          }}
        />
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark ml-3",
            label: "612Sl1",
          }}
        />
      </div>
      <div className="position-absolute single-text-6">
        <BlackContainer
          data={{
            label: "PP2_612BF1_DPT",
            useClass: "greenTxt",
            unit: "mmWG",
            unitColor: "unitColor",
          }}
        />
        <BlackContainer
          data={{
            label: "PP2_612BF1_PT",
            useClass: "greenTxt mt-2",
            unit: "bar",
            unitColor: "unitColor",
          }}
        />
        <BlackContainer
          data={{
            label: "PP2_612BL3_PT",
            useClass: "greenTxt mt-2",
            unit: "bar",
            unitColor: "unitColor",
          }}
        />
      </div>
      <div className="position-absolute single-text-7 text-center">
        <div>
          <MotorCircleDigitalTag className={`line-circle digital-tag ${useMotorTagsColorStatusForNewPlantScreens({ runLabel: "NA", healthyLabel: "NA" })}`} label="NA" />
        </div>
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark mt-1",
            label: "612FN1",
          }}
        />
      </div>
      <div className="position-absolute single-text-8 d-flex">
        <div>
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "612BL4",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612BL4_PT",
              useClass: "greenTxt mt-2",
              unit: "bar",
              unitColor: "unitColor",
            }}
          />
        </div>
        <div>
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "612HR2",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612HR2_TE",
              useClass: "greenTxt mt-2",
              unit: "°C",
              unitColor: "unitColor",
            }}
          />
        </div>
      </div>
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-9" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-10" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-11" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-12" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-13" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-14" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-15" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-16" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-17" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-18" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-19" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-20" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-21" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-22" />
      <PipeWithCircles parentDivClass="hpipecircle position-absolute single-text-23" />
      <div className="position-absolute single-text-24">
        <BlackContainer
          data={{
            label: "PP2_612BI1_WI",
            useClass: "greenTxt",
            unit: "t",
            unitColor: "unitColor",
          }}
        />
        <BlackContainer
          data={{
            label: "PP2_612BI1_WI_B",
            useClass: "greenTxt mt-1",
            unit: "%",
            unitColor: "unitColor",
          }}
        />
      </div>
      <div className="position-absolute single-text-25 p-1 bg-secondary box-1">
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark",
            label: "Force Down",
          }}
        />
        <div className="mt-1 d-flex">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mr-1",
              label: "SP",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612CG7_SP",
              useClass: "greenTxt",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
        </div>
        <div className="mt-1 d-flex">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mr-1",
              label: "PV",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612CG7_ZT",
              useClass: "greenTxt",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
        </div>
        <div className="mt-1 d-flex">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mr-1",
              label: "Out",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612CG7_SPSPM",
              useClass: "greenTxt",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
        </div>
      </div>
      <div className="position-absolute single-text-26 p-1 bg-secondary box-1">
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark",
            label: "Force UP",
          }}
        />
        <div className="mt-1 d-flex">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mr-1",
              label: "SP",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612CGB_SP",
              useClass: "greenTxt",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
        </div>
      </div>
      <BlackContainer
        data={{
          label: "PP2_612BF4_DPT",
          useClass: "greenTxt mt-1 position-absolute single-text-27",
          unit: "mmWG",
          unitColor: "unitColor",
        }}
      />
      <div className="position-absolute single-text-28 p-1 bg-secondary box-1">
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark",
            label: "Balance",
          }}
        />
        <div className="mt-1 d-flex">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mr-1",
              label: "SP",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612CGA_SP",
              useClass: "greenTxt",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
        </div>
      </div>
      <div className="position-absolute single-text-29 d-flex">
        <div>
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "612SGA",
            }}
          />
          <MotorCircleDigitalTag
            className={`line-circle mt-2 digital-tag ${useDampersValveTagsColorStatusForNewPlantScreens({ openLabel: "PP2_612SGA_OLS", closeLabel: "PP2_612SGA_CLS" })}`}
            label="PP2_612SGA_OLS"
          />
        </div>
        <div>
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "612SG9",
            }}
          />
          <MotorCircleDigitalTag
            className={`line-circle mt-2 digital-tag ${useDampersValveTagsColorStatusForNewPlantScreens({ openLabel: "PP2_612SG9_OLS", closeLabel: "PP2_612SG9_CLS" })}`}
            label="PP2_612SG9_OLS"
          />
        </div>
      </div>
      <div className="position-absolute single-text-30 p-1 bg-secondary box-1">
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark",
            label: "Force Down",
          }}
        />
        <div className="mt-1 d-flex">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mr-1",
              label: "PV",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612CG9_ZT",
              useClass: "greenTxt",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
        </div>
      </div>
      <div className="position-absolute single-text-31 p-1 bg-secondary box-1">
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark",
            label: "Balance",
          }}
        />
        <div className="mt-1 d-flex">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mr-1",
              label: "SP",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612CGB_SP",
              useClass: "greenTxt",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
        </div>
      </div>
      <div className="position-absolute single-text-32 p-1 bg-secondary box-1">
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark",
            label: "Force Down",
          }}
        />{" "}
        <div className="mt-1 d-flex">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mr-1",
              label: "PV",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612CG8_ZT",
              useClass: "greenTxt",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
        </div>
      </div>
      <div className="pipe dotted-pipe position-absolute single-text-33"></div>
      <div className="pipe dotted-pipe position-absolute single-text-34"></div>
      <div className="pipe dotted-pipe position-absolute single-text-35"></div>
      <div className="position-absolute single-text-36">
        <BlackContainer
          data={{
            label: "PP2_612BF2_DPT",
            useClass: "greenTxt mt-1",
            unit: "mmWG",
            unitColor: "unitColor",
          }}
        />
        <BlackContainer
          data={{
            label: "PP2_612BF2_PT",
            useClass: "greenTxt mt-2",
            unit: "bar",
            unitColor: "unitColor",
          }}
        />
      </div>
      <div className="position-absolute single-text-37 text-center">
        <div className="yellowline-Vr upArrow position-absolute line-1"></div>
        <div>
          <MotorCircleDigitalTag className={`line-circle digital-tag ${useMotorTagsColorStatusForNewPlantScreens({ runLabel: "NA", healthyLabel: "NA" })}`} label="NA" />
        </div>
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark mt-1",
            label: "612FN5",
          }}
        />
      </div>
      <div className="position-absolute single-text-38 text-center">
        <div className="yellowline-Vr upArrow position-absolute line-1"></div>
        <div>
          <MotorCircleDigitalTag className={`line-circle digital-tag ${useMotorTagsColorStatusForNewPlantScreens({ runLabel: "NA", healthyLabel: "NA" })}`} label="NA" />
        </div>
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark",
            label: "612FN7",
          }}
        />
      </div>
      <div className="position-absolute single-text-39 d-flex">
        <div className="p-1 bg-secondary box-1">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "Balance",
            }}
          />
          <div className="mt-1 d-flex">
            <TextContainerWithWrapWord
              data={{
                useClass: "text-dark mr-1",
                label: "SP",
              }}
            />
            <BlackContainer
              data={{
                label: "PP2_612CGD_SP",
                useClass: "greenTxt",
                unit: "%",
                unitColor: "unitColor",
              }}
            />
          </div>
        </div>
        <div className="p-1 bg-secondary box-1">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "Balance",
            }}
          />
          <div className="mt-1 d-flex">
            <TextContainerWithWrapWord
              data={{
                useClass: "text-dark mr-1",
                label: "SP",
              }}
            />
            <BlackContainer
              data={{
                label: "PP2_612CGG_SP",
                useClass: "greenTxt",
                unit: "%",
                unitColor: "unitColor",
              }}
            />
          </div>
        </div>
        <div className="p-1 bg-secondary box-1">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "Balance",
            }}
          />
          <div className="mt-1 d-flex">
            <TextContainerWithWrapWord
              data={{
                useClass: "text-dark mr-1",
                label: "SP",
              }}
            />
            <BlackContainer
              data={{
                label: "PP2_612CGK_SP",
                useClass: "greenTxt",
                unit: "%",
                unitColor: "unitColor",
              }}
            />
          </div>
        </div>
      </div>
      <div className="position-absolute single-text-40 d-flex">
        <div>
          <PipeWithCircles parentDivClass="hpipecircle mt-2" />
          <PipeWithCircles parentDivClass="hpipecircle mt-5 ml-2" />
          <PipeWithCircles parentDivClass="hpipecircle mt-5 ml-3" />
        </div>
        <div>
          <PipeWithCircles parentDivClass="hpipecircle mt-2" />
          <PipeWithCircles parentDivClass="hpipecircle mt-5 ml-2" />
          <PipeWithCircles parentDivClass="hpipecircle mt-5 ml-3" />
        </div>
        <div>
          <PipeWithCircles parentDivClass="hpipecircle mt-2" />
          <PipeWithCircles parentDivClass="hpipecircle mt-5 ml-2" />
          <PipeWithCircles parentDivClass="hpipecircle mt-5 ml-3" />
        </div>
      </div>
      <div className="position-absolute single-text-41 d-flex">
        <div>
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "612SGD",
            }}
          />
          {/* <TransparentBoxWithGate
            isClickable={false}
            parentClass="mx-auto mt-2"
            squareProps={{
              tagLabel: "",
              squareClass: `grey-color`,
            }}
          /> */}
          <MotorCircleDigitalTag
            className={`line-circle mt-2 digital-tag ${useDampersValveTagsColorStatusForNewPlantScreens({ openLabel: "PP2_612SGD_OLS", closeLabel: "PP2_612SGD_CLS" })}`}
            label="PP2_612SGD_OLS"
          />
        </div>
        <div>
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "612SGK",
            }}
          />
          {/* <TransparentBoxWithGate
            isClickable={false}
            parentClass="mx-auto mt-2"
            squareProps={{
              tagLabel: "",
              squareClass: `grey-color`,
            }}
          /> */}
          <MotorCircleDigitalTag
            className={`line-circle mt-2 digital-tag ${useDampersValveTagsColorStatusForNewPlantScreens({ openLabel: "PP2_612SGK_OLS", closeLabel: "PP2_612SGK_CLS" })}`}
            label="PP2_612SGK_OLS"
          />
        </div>
      </div>
      <div className="position-absolute single-text-42 text-center">
        <div className="yellowline-Vr upArrow position-absolute line-1"></div>
        <div>
          <MotorCircleDigitalTag className={`line-circle digital-tag ${useMotorTagsColorStatusForNewPlantScreens({ runLabel: "NA", healthyLabel: "NA" })}`} label="NA" />
        </div>
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark",
            label: "612FNJ",
          }}
        />
      </div>
      <div className="position-absolute single-text-43 d-flex">
        <div>
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "SEQUENCE",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612AE1_SEQ01",
              useClass: "greenTxt mt-1",
              unit: "",
              unitColor: "unitColor",
            }}
          />
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mt-2",
              label: "Cycle Timer",
            }}
          />
          <BlackContainer
            data={{
              label: "PP2_612AE1_SEQ01_SP",
              useClass: "greenTxt mt-1",
              unit: "s",
              unitColor: "unitColor",
            }}
          />
        </div>
        <div className="containerImage text-center">
          <BlackContainer
            data={{
              label: "PP2_611BI1_WI",
              useClass: "greenTxt mt-1",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "611BI1",
            }}
          />
        </div>
        <div className="containerImage text-center">
          <BlackContainer
            data={{
              label: "PP2_611BI2_WI",
              useClass: "greenTxt mt-1",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "611BI2",
            }}
          />
        </div>
        <div className="containerImage text-center">
          <BlackContainer
            data={{
              label: "PP2_611BI3_WI",
              useClass: "greenTxt mt-1",
              unit: "%",
              unitColor: "unitColor",
            }}
          />
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark",
              label: "611BI3",
            }}
          />
        </div>
      </div>
      <div className="position-absolute single-text-44 d-flex">
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark mt-1",
            label: "611ASG",
          }}
        />
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark mt-1",
            label: "611ASF",
          }}
        />
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark mt-1",
            label: "611ASE",
          }}
        />
      </div>
      <div className="position-absolute single-text-45 d-flex">
        <div>
          <MotorCircleDigitalTag
            className={`line-circle digital-tag ${useDampersValveTagsColorStatusForNewPlantScreens({ openLabel: "PP2_612SG6_OLS", closeLabel: "PP2_612SG6_CLS" })}`}
            label="PP2_612SG6_OLS"
          />
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mt-2",
              label: "SG6",
            }}
          />
        </div>
        <div className="tringle">
          <TextContainerWithWrapWord
            data={{
              useClass: "text-dark mt-1 ",
              label: "612BVE",
            }}
          />
          <MimicCaret
            parentClass="tringle-item"
            firstCaretClass={`fa-solid fa-caret-down d-block digital-tag ${useBVTagsColorStatusForNewPlantScreens({ openLabel: "PP2_612BVE_OLS", closeLabel: "PP2_612BVE_CLS" })}`}
            secondCaretClass={`fa-solid fa-caret-up d-block digital-tag ${useBVTagsColorStatusForNewPlantScreens({ openLabel: "PP2_612BVE_OLS", closeLabel: "PP2_612BVE_CLS" })}`}
            label="PP2_612BVE_OLS"
          />
        </div>
      </div>
      {/* <div className="position-absolute single-text-46 d-flex">
        <TransparentBoxWithGate
          isClickable={true}
          parentClass="mx-auto"
          squareProps={{
            tagLabel: "NA",
            squareClass: `${useColorStatus("NA")}`,
          }}
        />
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark mt-2 ml-2",
            label: "612PG1",
          }}
        />
      </div> */}
      <div className="position-absolute single-text-47">
        <TransparentBoxWithGate
          isClickable={true}
          parentClass="mx-auto"
          squareProps={{
            tagLabel: "PP2_612PG2_OLS",
            squareClass: `${useDampersValveTagsColorStatusForNewPlantScreens({openLabel: "PP2_612PG2_OLS", closeLabel:"PP2_612PG2_CLS"})}`,
          }}
        />
        <TextContainerWithWrapWord
          data={{
            useClass: "text-dark mt-2 ml-2",
            label: "612PG2",
          }}
        />
      </div>
      <div className="yellowline-Vr downArrow position-absolute line-1"></div>
      <div className="yellowline-Vr downArrow position-absolute line-2"></div>
      <div className="yellowline-Vr downArrow position-absolute line-3"></div>
      <div className="yellowline-Vr downArrow position-absolute line-4"></div>
      <div className="yellowline-Vr downArrow position-absolute line-5"></div>
      <div className="yellowline-Vr downArrow position-absolute line-6"></div>
      <div className="yellowline-Vr downArrow position-absolute line-7"></div>
      <div className="yellowline-Vr downArrow position-absolute line-8"></div>
      <div className="yellowline-Vr downArrow position-absolute line-9"></div>
      <div className="yellowline-Vr downArrow position-absolute line-10"></div>
      <div className="yellowline-Vr downArrow position-absolute line-11"></div>
      <div className="yellowline-Vr downArrow position-absolute line-12"></div>
      <div className="yellowline-Vr downArrow position-absolute line-13"></div>
      <div className="yellowline-Vr downArrow position-absolute line-14"></div>
      <div className="yellowline-Vr downArrow position-absolute line-15"></div>
      <div className="yellowline-Vr downArrow position-absolute line-16"></div>
      <div className="yellowline-Vr downArrow position-absolute line-17"></div>
      <div className="yellowline-Vr downArrow position-absolute line-18"></div>
      <div className="yellowline-Vr downArrow position-absolute line-19"></div>
      <div className="yellowline-Vr downArrow position-absolute line-20"></div>
      <div className="yellowline-Vr downArrow position-absolute line-21"></div>
      <div className="yellowline-Vr downArrow position-absolute line-22"></div>
      <div className="yellowline-Hr leftarrow position-absolute line-23"></div>
      <div className="yellowline-Hr leftarrow position-absolute line-24"></div>
      <div className="yellowline-Hr leftarrow position-absolute line-25"></div>
      <div className="yellowline-Vr downArrow position-absolute line-26"></div>
      <div className="yellowline-Vr downArrow position-absolute line-27"></div>
      <div className="yellowline-Vr downArrow position-absolute line-28"></div>
      <div className="yellowline-Vr downArrow position-absolute line-29"></div>
      <div className="yellowline-Vr downArrow position-absolute line-30"></div>
      <div className="yellowline-Vr downArrow position-absolute line-31"></div>
      <div className="yellowline-Vr downArrow position-absolute line-32"></div>
      <div className="yellowline-Vr downArrow position-absolute line-33"></div>
      <div className="yellowline-Vr downArrow position-absolute line-34"></div>
      <div className="yellowline-Vr downArrow position-absolute line-35"></div>
    </div>
  );
};
